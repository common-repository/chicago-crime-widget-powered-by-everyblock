<?php
/*
Plugin Name: Chicago Crime Widget Powered By Everyblock 
...
class ChicagoCrimeWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "crime";
	
	public function ChicagoCrimeWidget()
	{
		$widget_ops = array('classname' => 'ChicagoCrimeWidget', 'description' => 'Embeds a Chicago Crime Widget' );
		$this->WP_Widget('ChicagoCrimeWidget', 'Chicago Crime Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagoCrimeWidget");') );

function display_chicago_crime_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_crime_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagoCrimeWidget::$metro . "&schema=" . ChicagoCrimeWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_crime(stripslashes($code));
}

function get_html_for_embed_chicago_crime($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_crime_widget', display_chicago_crime_widget);
